...
import com.example.springstarbucksapi.repository.CashierOrderRepository;

import com.example.springstarbucksapi.config.WebConfig;
...
public class StarbucksService {
    ...
    @Autowired private CashierOrderRepository cashierOrderRepository;
    ...
    public StarbucksOrder newOrder(String regid, StarbucksOrder order) throws ResponseStatusException {
        ...
        if (!active.isEmpty()) {
            StarbucksOrder activeOrder = active.get(active.size() - 1);
            System.out.println("Active Order (Reg ID = " + regid + ") => " + activeOrder);
    ...
    public StarbucksOrder getActiveOrder(String regid) {
        List<StarbucksOrder> active = ordersRepository.findByRegister(regid);
        StarbucksOrder activeOrder = active.isEmpty() ? null : active.get(active.size() - 1);
        return activeOrder;
    }
